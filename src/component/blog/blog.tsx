import { Row, Col, Typography, Image, Flex, Input, Empty } from "antd";
...
const Blog = () => {
    ...
    return (
        <>
            <Input size="large" placeholder="หาที่เที่ยวเเล้วไปกัน" value={keyword} onChange={(e) => setKeyword(e.target.value)} style={{ marginBottom: 16 }} />
            {data.length > 0 ?
                data && data?.map((m) => {
                    return (
